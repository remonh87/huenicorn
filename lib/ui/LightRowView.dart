...
class LightRowView extends StatefulWidget {
  LightRowView({Key key, this.bridgeState, this.light}) : super(key: key);

  final BridgeState bridgeState;
  final Light light;
  final Key lightSwitchKey = new Key("hue_light_switch");
  final Key brightnessSliderKey = new Key("hue_brightness_slider");
...
class _LightRowViewState extends State<LightRowView> {
  ...
  Widget build(BuildContext context) {
    _hsvColor = new HSVColor.fromAHSV(
        1.0, widget.light.hue, widget.light.saturation, 1.0);
    return new Padding(
      padding:
            ...
            new ListTile(
              ...
              onTap: () {
                showDialog(
                    context: context,
                    child:
                        new LightColorPicker(widget.light, widget.bridgeState));
              },
              trailing: _createLightSwitch(),
            ),
            new Row(
              children: <Widget>[
                new Expanded(child: _createSlider()),
              ],
            )
          ])),
    );
  ...
  Widget _createLightIcon() {
    ...
    if (widget.light.isOn) {
      _lightIcon = new CircleAvatar(
        ...
        child: new ImageIcon(new AssetImage("assets/bulbs_black.png"),
            color: _hsvColor.toColor(), size: 30.0),
      );
    } else {
      _lightIcon = new CircleAvatar(
        backgroundColor: Colors.grey[850],
  ...
  Widget _createSlider() {
    ...
    if (widget.light.isOn) {
      _slider = new Slider(
          key: widget.brightnessSliderKey,
